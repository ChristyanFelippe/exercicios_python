...
def cadastro(): #Função que realiza a criação da lista com os dados
    list = []
    for i in range(4):
        dicionario = dict(nome = input('Digite seu nome: '),sexo = input('Digite M para masculino e F para feminino: '),
        esporte = input('Escolha seu esporte favorito entre natacao, futebol, volei,tenis: '), idade = int(input('Digite sua idade: ')))
...
for i in range(4):
 ...
 # Ex:lista[i]['esporte'] estou pegando a pessoa do indice i dalista e dentro dela usando a chave 'esporte' para acessar
 # o elemento
